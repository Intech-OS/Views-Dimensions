...
def save_one_box(xyxy, im, file=Path("im.jpg"), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    if crop.shape[0] > crop.shape[1]:  # If height is greater than width
        crop = np.rot90(crop, -1)  # Rotate the image

    #this code is to sharpen the blur image

    sharpening_kernel = np.array([[0, -1, 0], [-1, 5, -1], [0, -1, 0]]) 
    
    crop = cv2.filter2D(crop, -1, sharpening_kernel) 

    if BGR:
        crop = cv2.cvtColor(crop, cv2.COLOR_BGR2RGB)  # Convert BGR to RGB if necessary
    ...
    if save:
        ...
        cv2.imwrite(f, crop if BGR else cv2.cvtColor(crop, cv2.COLOR_RGB2BGR))
        Image.fromarray(crop if not BGR else crop[..., ::-1]).save(f, quality=95, subsampling=0)  
    return crop